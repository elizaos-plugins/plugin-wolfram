...
const wolframEnvSchema = z.object({
  ...
  WOLFRAM_CLOUD_API_KEY: z.string().optional().describe('Wolfram Cloud API Key for advanced features'),
  
  // Optional: Custom endpoint for Wolfram services
  WOLFRAM_API_ENDPOINT: z.string().url().optional().default('https://api.wolframalpha.com/v2'),
  
  // Optional: Response format preferences
  WOLFRAM_OUTPUT_FORMAT: z.enum(['plaintext', 'image', 'mathml', 'sound', 'wav']).optional().default('plaintext'),
  ...
  WOLFRAM_LOCATION: z.string().optional().describe('Location for location-based queries (e.g., "New York, NY")'),
  
  // Optional: Scanner types to use
  WOLFRAM_SCANNERS: z.string().optional().describe('Comma-separated list of scanner types to use'),
